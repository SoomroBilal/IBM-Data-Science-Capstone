...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        # Counting the success and failure rates for all sites
        data = spacex_df[spacex_df['class']==1]['Launch Site'].value_counts().reset_index()
        fig = px.pie(data,values='count',names = launch_sites,  title='Total Success Launches By Sites')
    else:
        # Counting the success and failure rates for the selected site
        data = spacex_df[spacex_df['Launch Site'] == entered_site]['class'].value_counts().reset_index()
        fig = px.pie(data, values='count',names = 'class',   title=f'Success vs Failure for {entered_site}')
    return fig
